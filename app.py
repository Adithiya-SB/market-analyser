...
def analyze():
    ...
    if isinstance(api_response, dict) and 'error' in api_response:
        print(f"API Error Detected: {api_response.get('error')}. Failing back to mock sentiment.")
        # FALLBACK: If API fails (e.g. rate limit, model loading), provide a simulated analysis
        # rather than crashing the UI with a 503 error.
        bull_count = sum(1 for kw in BULLISH_KEYWORDS if kw in text.lower())
        bear_count = sum(1 for kw in BEARISH_KEYWORDS if kw in text.lower())
        ...
        if bull_count > bear_count:
            ...
            label, score = 'neutral', 0.90
            
        api_response = [[{'label': label, 'score': score}]]
